...
const sharedModules = require('./shared-deps');
const paths = require('./paths');
const devPaths = require('./paths-dev');
...
module.exports = function (webpackEnv, isCordova) {
  ...
  // We will provide `paths.devPaths.publicUrlOrPath` to our app
  // as %PUBLIC_URL% in `index.html` and `process.env.PUBLIC_URL` in JavaScript.
  // Omit trailing slash as %PUBLIC_URL%/xyz looks better than %PUBLIC_URL%xyz.
  // Get environment variables to inject into our app.
  const env = getClientEnvironment(devPaths.publicUrlOrPath.slice(0, -1));
  ...
  const getStyleLoaders = (cssOptions, preProcessor) => {
    const loaders = [
      ...
      isEnvProduction && {
        ...
        // in production `paths.devPaths.publicUrlOrPath` can be a relative path
        options: devPaths.publicUrlOrPath.startsWith('.') ? { publicPath: '../../' } : {},
      },
      {
        ...
        },
        {
          ...
          options: {
            ...
            additionalData: (content, loaderContext) => {
              if (loaderContext.resourcePath.endsWith('abstract/_variables.scss')) {
                return content;
              }
              return `
  ...
  return {
    ...
    entry: {
      main: ['regenerator-runtime/runtime', paths.appIndexJs],
    },
    output: {
      // The build folder.
      path: paths.appBuild,
      ...
      filename: isEnvProduction ? 'static/js/[name].' + VERSION + '.js' : isEnvDevelopment && 'static/js/[name].js',
      // There are also additional JS chunk files if you use code splitting.
      chunkFilename: isEnvProduction ? 'static/js/[name].' + VERSION + '.chunk.js' : isEnvDevelopment && 'static/js/[name].chunk.js',
      assetModuleFilename: 'static/media/[name].' + VERSION + '[ext]',
      // webpack uses `publicPath` to determine where the app is being served from.
      // It requires a trailing slash, or the file assets will get an incorrect path.
      // We inferred the "public path" (such as / or /my-project) from homepage.
      publicPath: devPaths.publicUrlOrPath,
